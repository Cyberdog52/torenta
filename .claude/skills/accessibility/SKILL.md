...
**Conformance note:** This skill supports the development process; it does not by itself guarantee WCAG conformance. Automated tools and this checklist catch a subset of issues — manual keyboard/screen-reader testing and human review remain essential.

## Instructions

1. Identify which principle(s) and cross-cutting concerns the request touches (images, color/contrast, keyboard/focus, forms, timing/motion, language, ARIA/live regions) and jump to the matching section below.
2. Prefer semantic HTML first; add ARIA only when no native element/attribute provides the needed semantics (see [ARIA usage](#aria-usage-412)).
3. If the work involves generating or reviewing a rendered page, follow the [evidence-led audit workflow](#evidence-led-audit-workflow) below rather than guessing from source alone.
4. If color values are involved (new palette, theme, button/badge states), run the [contrast-check script](#automated-contrast-checking) against the actual hex values before finalizing.
5. After presenting code or fixes, list the specific accessibility decisions made — one line per component or pattern (e.g. "Modal: focus moves to heading on open, trapped while open, returns to trigger on close; Button: added `aria-pressed` for toggle state").
...
Rather than eyeballing hex values, validate real colors with the bundled script. Build a JSON palette of foreground/background pairs (see `assets/color-contrast-template.json` for the shape), then run:

```bash
node scripts/check-color-contrast.js assets/color-contrast-template.json
# or point it at your own palette file:
node scripts/check-color-contrast.js assets/my-component-palette.json
```

The script computes the WCAG relative-luminance contrast ratio for each pair, applies the correct threshold (4.5:1 / 3:1 text rules, or 3:1 for `isUiComponent: true` per 1.4.11 Non-text Contrast), prints a per-check pass/fail report, and exits non-zero if anything fails — safe to wire into a pre-commit hook or CI step.
...
# color contrast (bundled script, no live page needed)
node scripts/check-color-contrast.js assets/color-contrast-template.json
```
...
- [Contrast-check script](scripts/check-color-contrast.js) and [palette template](assets/color-contrast-template.json)
